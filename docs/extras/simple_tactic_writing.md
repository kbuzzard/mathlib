# Writing tactics without learning tactics #
...
def F : ℕ → ℕ → ℕ → Prop := sorry 

lemma lemma_id {a b c : ℕ} : F a b c = F a b c := rfl
lemma lemma_23 {a b c : ℕ} : F a b c = F a c b := sorry
lemma lemma_12 {a b c : ℕ} : F a b c = F b a c := sorry 
lemma lemma_123 {a b c : ℕ} : F a b c = F b c a := by rw [@lemma_12 a b c,lemma_23]
lemma lemma_13 {a b c : ℕ} : F a b c = F c b a := by rw [@lemma_123 a b c,lemma_12]
lemma lemma_132 {a b c : ℕ} : F a b c = F c a b := by rw [@lemma_123 a b c,lemma_123]

lemma test {a b c : ℕ} : F a b c = F c b a:=
begin
  exact lemma_id <|> exact lemma_23 <|> exact lemma_12 
    <|> exact lemma_123 <|> exact lemma_13 <|> exact lemma_132
end 

-- don't ask me any questions about what's going on here
meta def lazy : tactic unit := do
`[exact lemma_id <|> exact lemma_23 <|> exact lemma_12 
    <|> exact lemma_123 <|> exact lemma_13 <|> exact lemma_132]

lemma test2 {a b c : ℕ} : F a b c = F b c a := by lazy -- works!
