...
-- TODO -- ask Mario about useless import
-- and document why other imports needed
import order.conditionally_complete_lattice
import data.real.cau_seq
--import algebra.big_operators -- useless
...
def real := @quotient (cau_seq ℚ abs) cau_seq.equiv -- clearer for me
--example : quotient (@cau_seq.equiv ℚ _ _ _ abs _) = real := rfl 
...
--#check rat.mk
-- TODO I don't understand how we know what's going on here
-- is it because there's only one equiv reln on R?
...
-- the mathematician's proof is "it follows from add_lim_zero"
-- The computer scientist writes three lines of introduction
...
⟨λ x y, quotient.lift_on₂ x y (λ f g, mk (f + g)) $
  ...
  /-
  begin 
  have ZZZ := add_lim_zero hf hg,
  unfold has_equiv.equiv setoid.r,
  suffices : f₁ + g₁ - (f₂ + g₂) = f₁ - f₂ + (g₁ - g₂),
  rwa this,
...
#check (by apply_instance : has_lt (cau_seq ℚ abs))
#print cau_seq.has_lt
#print cau_seq.pos
#print cau_seq
#print is_cau_seq 
